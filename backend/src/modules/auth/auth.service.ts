...
export async function login(
  ...
  const company = await prisma.company.findFirst({
    where: { code: companyCode, status: "ACTIVE", deletedAt: null },
  });
  if (!company)
    throw new AppError(
      401,
  ...
  const user = await prisma.user.findFirst({
    where: { companyId: company.id, email, deletedAt: null },
  ...
  if (!(await bcrypt.compare(password, user.passwordHash))) {
    ...
    await prisma.user.update({
      ...
      data: {
        failedLoginAttempts,
        lockedUntil:
          failedLoginAttempts >= MAX_FAILURES
  ...
  return prisma.$transaction(async (tx) => {
    await tx.userSession.updateMany({
      where: { companyId: company.id, userId: user.id, revokedAt: null },
    ...
    const provisional = await tx.userSession.create({
      data: {
...
export async function refresh(rawToken: string) {
  ...
  try {
    ...
    throw new AppError(401, "REFRESH_INVALID", "Token de renovação inválido.");
  }
  ...
  if (
    ...
    throw new AppError(401, "SESSION_REVOKED", "Sessão revogada ou expirada.");
  const refreshToken = signRefresh({
    sub: claims.sub,
    companyId: claims.companyId,
  ...
  await prisma.userSession.update({
    where: { id: session.id },
    data: { tokenHash: hashToken(refreshToken), lastUsedAt: new Date() },
  });
  return {
    accessToken: signAccess({
      sub: claims.sub,
...
export async function currentUser(userId: string, companyId: string) {
  ...
  return {
    user: {
      ...
      name: user.name,
      email: user.email,
      role: user.roles[0]?.role.name ?? "Usuario",
