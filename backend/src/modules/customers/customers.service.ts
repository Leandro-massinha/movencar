    ...
    ...(input.search
      ? {
          OR: [
            ...
            { document: { contains: normalizedSearch || input.search } },
            { phone: { contains: normalizedSearch || input.search } },
            { whatsapp: { contains: normalizedSearch || input.search } },
...
export async function updateCustomer(
  ...
  const current = await getCustomer(actor.companyId, id);
  await assertBranch(actor.companyId, input.originBranchId);
  const resultingType = input.type ?? current.type;
  const resultingDocument =
    input.document === undefined ? current.document : input.document;
  if (
    resultingDocument &&
    resultingDocument.length !== (resultingType === "COMPANY" ? 14 : 11)
  ) {
    throw new AppError(
      400,
      "INVALID_CUSTOMER_DOCUMENT",
      resultingType === "COMPANY"
