...
const seed = () => { const store = new CustomerStore(); store.create('A', { id: 'a1', name: 'Ana Cliente', document: '12345678901', branchId: 'a-main', status: 'ACTIVE' }); return store }

describe('customer validation', () => {
  it('normalizes formatted CPF and whitespace', () => expect(createCustomerSchema.parse({ name: '  Ana   Maria ', document: '123.456.789-01' })).toMatchObject({ name: 'Ana Maria', document: '12345678901' }))
  it('normalizes CNPJ for company customers', () => expect(createCustomerSchema.parse({ name: 'Empresa', type: 'COMPANY', document: '12.345.678/0001-90' }).document).toBe('12345678000190'))
...
describe('customer tenant rules', () => {
  ...
  it('rejects duplicate active documents in one company', () => expect(() => seed().create('A', { id: 'a2', name: 'Duplicate', document: '12345678901', status: 'ACTIVE' })).toThrow('duplicate'))
  it('permits the same document in different companies', () => expect(seed().create('B', { id: 'b1', name: 'Other', document: '12345678901', status: 'ACTIVE' })?.companyId).toBe('B'))
  ...
  it('soft delete hides a customer and permits document reuse', () => { const store = seed(); expect(store.remove('A', 'a1')).toBe(true); expect(store.find('A', 'a1')).toBeUndefined(); expect(store.list('A').data).toHaveLength(0); expect(store.create('A', { id: 'a2', name: 'Recreated', document: '12345678901', status: 'ACTIVE' })).toBeDefined() })
  ...
  it('searches by normalized document', () => expect(seed().list('A', '12345678901').data[0].id).toBe('a1'))
