...
Somente `DRAFT` é editável. Conclusão usa compare-and-swap; o vencedor cria exatamente um `PDC_COMPLETED`, uma leitura de odômetro quando informada e um AuditLog. Quilometragem histórica menor é aceita sem reduzir `Vehicle.currentMileage`. PDC concluído é imutável pelas APIs normais.
