...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const [user, setUser] = useState<User | null>(() =>
    useMocks && sessionStorage.getItem("movencar.demo.session")
      ? demoUser
      : null,
  );
  ...
  const logout = useCallback(() => {
    if (!useMocks) void api.post("/auth/logout").catch(() => undefined);
    sessionStorage.removeItem("movencar.demo.session");
    setAccessToken(null);
    setUser(null);
  }, [useMocks]);
  useEffect(() => {
    window.addEventListener("movencar:session-revoked", logout);
    return () => window.removeEventListener("movencar:session-revoked", logout);
  }, [logout]);
  useEffect(() => {
    if (useMocks) return;
    api
  ...
  const login = useCallback(
    async (email: string, password: string) => {
      ...
      const companyCode =
        import.meta.env.VITE_COMPANY_CODE || "oficina-avenida";
  ...
  const setBranch = (branchId: string) =>
    setTenant((current) => ({
      ...current,
      branchId,
      branchName: branchId === "norte" ? "Unidade Norte" : "Matriz - Centro",
    }));
  return (
    <AuthContext.Provider
      value={useMemo(
        ...
        [user, tenant, initializing, login, logout],
      )}
    >
      {children}
    </AuthContext.Provider>
