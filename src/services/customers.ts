...
export const customersApi = {
  list: async (params: {
    ...
    if (!useMocks)
      return api
        .get<CustomerList>("/customers", { params })
        .then(({ data }) => data);
    const search =
      (params.search || "").toLowerCase().replace(/\D/g, "") ||
    ...
    const filtered = mockCustomers.filter(
      (customer) =>
        ...
        (!search ||
          `${customer.name} ${customer.document || ""} ${customer.email || ""}`
            .toLowerCase()
            .includes(search)),
    );
    return {
  ...
  create: async (input: CustomerInput) => {
    if (!useMocks)
      return api
        .post<{ customer: Customer }>("/customers", input)
