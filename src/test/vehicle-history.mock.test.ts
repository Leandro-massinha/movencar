...
describe("vehicle history mock mode", () => {
  it("creates, filters and paginates manual events without using the API", async () => {
    ...
    const created = await vehicleHistoryApi.create(vehicleId, {
      eventType: "NOTE",
      title: "  Nota mock  ",
    ...
    const listed = await vehicleHistoryApi.list(vehicleId, {
      page: 1,
      limit: 1,
      eventType: "NOTE",
      sortOrder: "desc",
  ...
  it("keeps histories isolated by vehicle in the mock adapter", async () => {
    await vehicleHistoryApi.create("mock-vehicle-private", {
      eventType: "GENERAL",
      title: "Evento privado",
    });
    const other = await vehicleHistoryApi.list("mock-vehicle-other", {
      page: 1,
      limit: 20,
      sortOrder: "desc",
    });
